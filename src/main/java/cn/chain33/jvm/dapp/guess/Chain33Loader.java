 ...
 */
public class Chain33Loader extends ClassLoader {

    private static final Chain33Loader INSTANCE = new Chain33Loader();

    public static final Chain33Loader getInstance() {
        return INSTANCE;
    }
    public Chain33Loader(){
        super();
    }
    /**
     ...
     * @return
     */
    public static int loadLibJar(String path) {
        List<File> list = INSTANCE.scanDir(path);
        for (File f : list) {
            JarFile jar;
            try {
                jar = new JarFile(f);
    ...
    public static void main(String[] args) {
        ...
        System.arraycopy(args, 1, progArgs
                , 0, progArgs.length);
        try {
            Chain33Loader loader = getInstance();
            loadLibJar("lib");
    ...
    public static int tx(String[] args) {
        ...
        if (!isExist(contractName)) {
            INSTANCE.loadContract(contractName);
        }
        try {
            Class<?> clazz = INSTANCE.loadClass(getEntryClass(contractName));
    ...
    private byte[] getClassFromMap(String name) {
        //load contract class
        if (this.contractClass.containsKey(name)) {
            return this.data.get(this.contractClass.get(name)).get(name);
        }
        //load common class
        if (this.common.containsKey(name)) {
        ...
        while (en.hasMoreElements()) {
            ...
            if (name.endsWith(".class")) {
                ...
                if (clss.endsWith(contractName)) {
                    this.entryClass.put(contractName, clss);
                }

                this.contractClass.put(clss, contractName);
                if (this.findLoadedClass(clss) != null) continue;
